...
/*! 取皮肤样式名，默认为 default */
        _skin = _getArgs('skin') || settings.skin || 'default',
	 ...
	 */
        position: function(left, top) {
            ...
            if (top || top === 0) {
                ...
                top = that._toNumber(top, wh - oh)+60;
	 ...
	 */
        ...
        lock: function() {
            ...
            var that = this, frm,
                ...
                config = that.config,
                mask = $('#ldg_lockmask_'+index, _doc)[0],
            ...
            if (!mask) {
                ...
                mask.id = 'ldg_lockmask_'+index;
                mask.style.cssText = 'background:#7f7f7f;filter:alpha(opacity=60);opacity:.6;position:' + positionType + ';left:0;top:0;width:100%;height:100%;overflow:hidden;';
            ...
            that._lock = true;
            that._lockId = 'ldg_lockmask_' + index;
            return that;
        },
            ...
            that._frmTimer = setTimeout(function() {
                if (config.isPost) {
                    var html = '<form action="' + url + '" method="post" target="_self" id="postData_form">' +
                        '<input id="wor_iteId" name="' + config.pname + '" type="hidden" value="' + config.pvalue + '"/>' +
                        '</form>';
...
/*! lhgdialog 的全局默认配置 */
    lhgdialog.setting =
    {
        ...
        isPost:false,             //是否POST提交
        pname:null,           //如果是POST提交，要提交的数据
        pvalue:null             //如果是POST提交，要提交的数据
    };
 ...
 */
    lhgdialog.confirm = function(content, yes, no, parent) {
        return lhgdialog({
            title: window.currentLang == 'zh-cn' ? '确认' : "Confirm",
            id: 'confirm.gif',
            zIndex: _zIndex(),
            icon: 'confirm.gif',
            fixed: true,
